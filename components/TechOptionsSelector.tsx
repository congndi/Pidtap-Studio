...
const TechOptionsSelector: React.FC<TechOptionsSelectorProps> = ({ techOptions, setTechOptions }) => {
    ...
    return (
        <div>
            <h3 className="text-base font-semibold text-gray-200 mb-4 border-t border-gray-700 pt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {Object.entries(techOptionsData).map(([key, options]) => (
                    <div key={key}>
                        <label htmlFor={`${key}-selector`} className="block text-sm font-medium text-gray-400 mb-1">
                            {techOptionLabels[key as keyof TechOptions]}
                        </label>
                        <select
                            id={`${key}-selector`}
                            className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-3 py-2 text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition"
